from style_change_method import incr_opr_usage
from style_change_method import incr_opr_prepost
...
def check_transform(auth_style, program_style, path_program, path_author, converted_styles):
    for key, value in program_style.items():
        if 0 < max(program_style.values()) == value:
            if auth_style != key:
...
def transform_random(program_style, program_name, save_path, converted_styles):
    for key, value in program_style.items():
        if 0 < max(program_style.values()) == value:
            converted_styles.append('10')
            incr_opr_prepost.program_transform_save_div(program_name, save_path, key)
            break
